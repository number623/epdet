...
def search_mp_NG ( img ) :
	height = img.shape[0]
	width  = img.shape[1]

	### search for left edge
	mp_l_det = False
	for mp_x in reversed( range( 0, width / 2 ) ) :
		is_all_0 = True
		for mp_y in range( height ) :
			if img[ mp_y, mp_x ] != 0 :
				is_all_0 = False
				break
		if is_all_0 == True :
	...
	mp_r_det = False
	for mp_x in range(  width/2, width ) :
		is_all_0 = True
		for mp_y in range( height ) :
			if img[ mp_y, mp_x ] != 0 :
				is_all_0 = False
	...
	### search for top edge
	mp_u_det = False
	for mp_y in reversed( range( 0, height / 2 ) ) :
		is_all_0 = True
		for mp_x in range( mp_l_x, mp_r_x+1 ) :
			if img[ mp_y, mp_x ] != 0 :
				is_all_0 = False
				break
		if is_all_0 == True :
	...
	if mp_u_det == False :
		...
		mp_diameter = ( mp_r_x - mp_l_x )
		mp_center_x = ( mp_l_x + mp_r_x ) // 2
		mp_center_y = ( mp_u_y + ( mp_diameter // 2 ) ) 
		return mp_center_x, mp_center_y, mp_diameter, True
